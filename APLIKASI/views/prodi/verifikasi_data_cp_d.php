			...
			<?php if(isset($cp_konten[$cp_title[$i]['KODE']])){ ?>
				...
				<?php for ($j=0; $j < count($cp_konten[$cp_title[$i]['KODE']]); $j++) { 
					...
					<tr style="text-align: justify; padding: 5px;">
						...
						<?php if(!$status_skpi){ ?> <td align="center" style="vertical-align: middle;"><input type="checkbox" value="<?php echo $cp_konten[$cp_title[$i]['KODE']][$j]['KODE']; ?>" name="capaian[]" <?php echo $validasi[$cp_konten[$cp_title[$i]['KODE']][$j]['KODE']]; ?>></td> <?php } ?>
					</tr>
